...
| `/` "Watch the trailer" button | **WIRE OR CUT** | No `/trailer` route, no video. Now `disabled` + `cursor: not-allowed` + `title="Not available yet"` so it no longer reads as live — but it still has to be wired or removed before launch. |
...
| `/login` + `/signup` | SHIPS | Real Supabase auth (`signInWithPassword` / `signUp`), return-url round trip, real errors surfaced, disabled with an honest notice when env vars are absent. **Not yet verified against the live backend** — egress to `*.supabase.co` is blocked from the build session, so one real sign-in on a deploy is still owed. |
...
| Collapse rail | SHIPS | |
| Header "Watch the Trailer" | **WIRE OR CUT** | A non-interactive `<span>`, dimmed to `white/25`, `title="Not available yet"`. Honest as-is; still wire or remove before launch. |

## 3. Destinations — 13 in the registry, 2 ported

| Destination | Route | Status | Detail |
|---|---|---|---|
| Zayra Home | `/empire` | **WIRE OR CUT** | Shell + cards ship. All six tabs show sign-in / not-available states. Needs per-user data. The "Private Alpha Status" card previously green-ticked five features that do not exist here; corrected to Available / In training / Locked. |
...
| Ask Zayra panel | **WIRE OR CUT** | Presence only. No chat. Needs the shared brain. Copy no longer leaks an internal design note. |
...
| Stage 1 "Your Idea" | **WIRE OR CUT** | Form works. **Nothing is saved at all** — `onSubmit` discards the values and flips a local boolean. Needs a destination table on the Lovable side. |
| Stage 4 "Formation" | **WIRE OR CUT** | Form works, spelling gates correct, lock banner renders. Same discard-on-submit gap as stage 1. |
...
| Auth (sign in / up) | SHIPS | `/login` exists and all three entry CTAs reach it. See the public-surfaces row for the verification still owed. |
...
Counted from the wiring tables in sections 1–6 — 44 rows in total (section 7
...
**Auth was the keystone, and the door now exists.** `/login` is built on real
Supabase auth and all three entry CTAs reach it; before that, every way in
dead-ended at an empty anchor div. What it does not yet do is prove itself: the
build session cannot reach `*.supabase.co`, so a real sign-in against the live
project is still owed before this can be called finished.

**The next keystone is persistence.** Zayra Home's six tabs, `/usage`, and both
working forms still have nowhere to read from or write to. The forms are the
starkest case: they discard their values on submit. That needs a destination
table on the Lovable side — this app does not create tables.
